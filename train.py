...
def train_model(config):
     ...
     loss_function = nn.CrossEntropyLoss(ignore_index=tokenizer_tgt.token_to_id("[PAD]"), label_smoothing=0.1).to(device)

     # resume from checkpoint in case of crash
     ...
     for epoch in range(initial_epoch, config['num_epochs']):
          ...
          for batch in batch_iterator:
               ...
               decoder_output = model.decode(decoder_input, encoder_output, encoder_mask, decoder_mask)     # shape: (batch_size, seq_len, d_model)  
               projection_output = model.project(decoder_output)                                            # shape: (batch_size, seq_len, vocab_tgt_size)
